...
int ut_dl_init_txd(struct m0_be_dtm0_log *log, struct m0_be_tx *tx,
                   struct m0_be_seg *seg, struct m0_dtm0_tx_desc *txd,
                   int rand)
{
	int i;
	int rc;

	rc = m0_dtm0_tx_desc_init(tx, seg, txd, UT_DTM0_LOG_MAX_PA,
                                  log->dl_ispstore);
	if (rc != 0)
		return rc;
...
void ut_dl_fini_txd(struct m0_be_dtm0_log *log, struct m0_be_tx *tx,
                    struct m0_be_seg *seg, struct m0_dtm0_tx_desc *txd)
{
	m0_dtm0_tx_desc_fini(tx, seg, txd, log->dl_ispstore);
}
...
int ut_dl_init(struct m0_be_dtm0_log *log, struct m0_be_tx *tx,
               struct m0_be_seg *seg, struct m0_dtm0_tx_desc *txd,
               struct m0_buf *buf, int rand)
{
	int rc;

	rc = ut_dl_init_txd(log, tx, seg, txd, rand);
	if (rc != 0)
		return rc;
...
void ut_dl_fini(struct m0_be_dtm0_log *log, struct m0_be_tx *tx,
                struct m0_be_seg *seg, struct m0_dtm0_tx_desc *txd,
                struct m0_buf *buf)
{
	ut_dl_fini_txd(log, tx, seg, txd);
	ut_dl_fini_buf(buf);
}
...
void test_volatile_dtm0_log(void)
{
	...
	for (i = 0; i < UT_DTM0_LOG_MAX_LOG_REC; ++i) {
		rc = ut_dl_init(log, tx, seg, &txd[i], &buf[i], i);
		M0_UT_ASSERT(rc == 0);
	}
	...
	rc = m0_be_dtm0_log_update(log, tx, seg, &txd[0], &buf[0]);
	M0_UT_ASSERT(rc == 0);
	...
	rc = m0_be_dtm0_log_prune(log, tx, seg, &txd[0].dtd_id);
	...
	ut_dl_set_pa_state(&txd[0].dtd_pg.dtpg_pa[2], M0_DTPS_PERSISTENT);
	rc = m0_be_dtm0_log_update(log, tx, seg, &txd[0], &empty_buf);
	M0_UT_ASSERT(rc == 0);
	rec[0] = m0_be_dtm0_log_find(log, &txd[0].dtd_id);
	M0_UT_ASSERT(rec[0] != NULL);
	rc = ut_dl_verify_log_rec(rec[0], 0);
	...
	for (i = 0; i < UT_DTM0_LOG_MAX_LOG_REC; ++i) {
		...
		ut_dl_set_pa_state(&txd[i].dtd_pg.dtpg_pa[2],
                                   M0_DTPS_PERSISTENT);
		rc = m0_be_dtm0_log_update(log, tx, seg, &txd[i], &buf[i]);
		M0_UT_ASSERT(rc == 0);
	...
	rc = m0_be_dtm0_log_prune(log, tx, seg, &txd[9].dtd_id);
	M0_UT_ASSERT(rc == 0);
	...
	/* Finalization */
	for (i = 0; i < UT_DTM0_LOG_MAX_LOG_REC; ++i) {
		ut_dl_fini(log, tx, seg, &txd[i], &buf[i]);
	}
