...
#define LIBFAB_SGL_LIMIT (FAB_TCP_SOCK_SGL_LIMIT > FAB_VERBS_SGL_LIMIT) ? \
					      FAB_TCP_SOCK_SGL_LIMIT : FAB_VERBS_SGL_LIMIT
			  ...
			  uint64_t *name_fmt_numeric)
{
	...
	M0_PRE(ep != NULL);
	if (name == NULL) {
		if(*name_fmt_numeric == ep->fep_name_fmt_numeric )
			ret = true;
	} else {
			  ...
			  struct m0_net_end_point **epp)
{
	...
	char                     *wc = NULL;
	int                       rc = 0;
	uint64_t                  name_fmt_numeric = 0;

	if( epn )
		libfab_ep_pton(epn,&name_fmt_numeric); 

	M0_ASSERT(libfab_tm_is_locked(tm->ntm_xprt_private));
	net = m0_tl_find(m0_nep, net, &tm->ntm_end_points,
			 libfab_ep_cmp(libfab_ep(net), name, &name_fmt_numeric));
...
static int libfab_active_ep_create(struct m0_fab__ep *ep, struct m0_fab__tm *tm)
{
	...
	libfab_ep_pton( &ep->fep_name, &ep->fep_name_fmt_numeric);
...
static void libfab_buf_done(struct m0_fab__buf *buf, int rc)
{
	...
	if (!fab_buf_tlink_is_in(buf)) {
		...
		if (m0_thread_self() == &ma->ftm_poller) {
			if( M0_FI_ENABLED("lf_dummy_msg_rcv") && (buf->fb_length == (sizeof(uint32_t) * 2)) ) {
							...
							bool is_verb )
{
	...
	while(xfer_len < fb->fb_nb->nb_length) {
		idx = 0;
		while(idx < iov_limit && !last_seg)
		{
			M0_ASSERT(rem_sidx <= fb->fb_rbd->fbd_iov_cnt);
			loc_slen = v_cnt[loc_sidx] - loc_soff;
			rem_slen = r_iov[rem_sidx].len - rem_soff;
		
			lf_iv[idx].iov_base = fb->fb_nb->nb_buffer.ov_buf[loc_sidx] + loc_soff;
			lf_iv[idx].iov_len = min64u(loc_slen, rem_slen);
			...
			if (loc_slen > rem_slen) {
				rem_sidx++;
				rem_soff = 0;
				loc_soff += lf_iv[idx].iov_len;
			} else {
				loc_sidx++;
				loc_soff = 0;
				rem_soff += lf_iv[idx].iov_len;
				if(rem_soff >= r_iov[rem_sidx].len) {
					rem_sidx++;
					rem_soff = 0;
				}
...
static int libfab_ma_start(struct m0_net_transfer_mc *ntm, const char *name)
{
	...
	if (ftm->ftm_pep != NULL) {
		...
		libfab_ep_pton( &ftm->ftm_pep->fep_name, &ftm->ftm_pep->fep_name_fmt_numeric);
