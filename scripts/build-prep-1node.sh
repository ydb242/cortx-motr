...
[[ -d cortx-motr ]] || {
    ...
    cd motr && git checkout sage-2.1 && git submodule update --init
    cd -
}
...
if [[ $dev_mode ]]; then
    ...
    ls -t ~/rpmbuild/RPMS/x86_64/cortx-motr{,-devel,-debuginfo}-2* | head -3 |
...
if [[ $dev_mode ]]; then
    ...
    ls -t ~/rpmbuild/RPMS/x86_64/cortx-hare{,-debuginfo}-2* | head -2 |
